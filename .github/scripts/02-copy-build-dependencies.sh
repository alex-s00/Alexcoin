...
if [[ ${GITHUB_REF} =~ "release" || ${FORCEBUILDDEPS} = "1" ]]; then
    ...
    cd /tmp
    curl -O https://alex-build-resources.s3.amazonaws.com/${OS}/alex-${OS}-dependencies.tar.gz
    curl -O https://alex-build-resources.s3.amazonaws.com/${OS}/SHASUM
    if [[ $(sha256sum -c /tmp/SHASUM) ]]; then
        cd ${GITHUB_WORKSPACE}/depends
        tar zxvf /tmp/alex-${OS}-dependencies.tar.gz
    else
