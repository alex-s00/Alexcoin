...
The RPM spec file provided here is for Alex-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://alex.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Alex git repository](https://github.com/AlexProject/Alexcoin) but are not part of
...
Currently only `Source10` is used, for the example `alex.conf` file.
...
in the alex git repository. At present only an SVG version of the Alex
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the alex
...
needed when 0.12.1 is released, a proper fix is already in the Alex git
master branch.
...
idea to build Alex Core against a static version of that library compiled
...
The `alex.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `alex-server` package. At this point, most RPM based
...
The `alex-server` package should have SELinux support. How to properly do
...
Alex-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/alex/). He has been
packaging Alex for Fedora at least since 2012.
...
4. I renamed the `alex` package that contains the Qt GUI to `alex-core` as
that appears to be how the general population refers to it, in contrast to
`alex-xt` or `alex-classic`. I wanted to make sure the general population
