...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for ALEX
    ...
    if (p_rewardSnapshot.strDistributionAsset != "ALEX") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is ALEX with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using ALEX differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "ALEX") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Alex address (already validated during ownership snapshot creation)
