 ...
 * between alexd, and alex-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateAlexs(false, 0, GetParams());
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/AlexProject/Alexcoin>";
    const std::string URL_WEBSITE = "<https://alexcoin.org>";
...
/** Sanity checks
 *  Ensure that Alex is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Alex process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    GenerateAlexs(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
