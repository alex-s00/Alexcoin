...
// Copyright (c) 2017-2021 The Alex Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/alex-config.h"
#endif
...
const std::string AlexGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString AlexGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "ALEXBTC"  , 1,          8},
    { "mBTC",   "ALEXBTC"  , 1000,       5},
    { "µBTC",   "ALEXBTC"  , 1000000,    2},
    { "Satoshi","ALEXBTC"  , 100000000,  0},
    { "USDT",   "ALEXUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(AlexGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

AlexGUI::AlexGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void AlexGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Alex address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and alex: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to ALEX addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Alex addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Alex addresses"));
    ...
    openAction->setStatusTip(tr("Open a alex: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Alex command-line options").arg(tr(PACKAGE_NAME)));
...
void AlexGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            ...
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/alexcointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* alexLabelLayout = new QVBoxLayout(toolbarWidget);
        alexLabelLayout->addWidget(labelToolbar);
        alexLabelLayout->addWidget(m_toolbar);
        alexLabelLayout->setDirection(QBoxLayout::TopToBottom);
        alexLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Alexcoin Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/AlexProject/Alexcoin/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Alexcoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/AlexProject/Alexcoin/releases";
...
void AlexGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/rvntext")));
        m_toolbar->setMaximumWidth(65);
...
void AlexGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool AlexGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool AlexGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void AlexGUI::setWalletActionsEnabled(bool enabled)
{
...
void AlexGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void AlexGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void AlexGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AlexGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void AlexGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void AlexGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void AlexGUI::openClicked()
{
...
void AlexGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AlexGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AlexGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AlexGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AlexGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AlexGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** ALEX START */
void AlexGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void AlexGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void AlexGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void AlexGUI::gotoRestrictedAssetsPage()
{
...
void AlexGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Alex network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void AlexGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AlexGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void AlexGUI::updateHeadersSyncProgressLabel()
{
...
void AlexGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void AlexGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Alex"); // default title
...
void AlexGUI::changeEvent(QEvent *e)
{
...
void AlexGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AlexGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AlexGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "ALEX")
        msg += tr("Amount: %1\n").arg(AlexUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(AlexUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void AlexGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AlexGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AlexGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AlexGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AlexGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void AlexGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AlexGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AlexGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AlexGUI::detectShutdown()
{
...
void AlexGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void AlexGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMnemonic(AlexGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void AlexGUI::subscribeToCoreSignals()
{
...
void AlexGUI::unsubscribeFromCoreSignals()
{
...
void AlexGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void AlexGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<AlexUnits::Unit> units = AlexUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AlexUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(AlexUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(AlexUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (AlexUnits::Unit u : AlexUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(AlexUnits::name(u)), this);
...
{
    setText(AlexUnits::name(newUnits));
}
...
void AlexGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AlexGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "AlexGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void AlexGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/AlexProject/Alexcoin/releases"));
