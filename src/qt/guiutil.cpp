...
#include "alexaddressvalidator.h"
#include "alexunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Alex address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new AlexAddressEntryValidator(parent));
    widget->setCheckValidator(new AlexAddressCheckValidator(parent));
}
...
bool parseAlexURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no alex: URI
    if(!uri.isValid() || uri.scheme() != QString("alex"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!AlexUnits::parse(AlexUnits::ALEX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseAlexURI(QString uri, SendCoinsRecipient *out)
{
    // Convert alex:// to alex:
    //
    //    Cannot handle this later, because alex:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("alex://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "alex:");
    }
    QUrl uriInstance(uri);
    return parseAlexURI(uriInstance, out);
}

QString formatAlexURI(const SendCoinsRecipient &info)
{
    QString ret = QString("alex:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AlexUnits::format(AlexUnits::ALEX, info.amount, false, AlexUnits::separatorNever));
        paramCount++;
    }
...
bool openAlexConf()
{
    ...
    /* Open alex.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Alex.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Alex (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Alex (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "alex.desktop";
    return GetAutostartDir() / strprintf("alex-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a alex.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Alex\n";
        else
            optionFile << strprintf("Name=Alex (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the alex app
...
bool GetStartOnSystemStartup()
{
    CFURLRef alexAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (alexAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, alexAppUrl);

    CFRelease(alexAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add alex app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, alexAppUrl, nullptr, nullptr);
    }
