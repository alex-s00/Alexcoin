...
The Alex Core daemon can be configured to act as a trusted "border
router", implementing the alex wire protocol and relay, making
...
The ZeroMQ feature in Alex Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building alexd:
...
For instance:
    ...
    $ alexd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/alexd.tx.raw
...
These options can also be provided in alex.conf.
...
From the perspective of alexd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into alexd directly. Furthermore, no information is
...
using. Alexd appends an up-counting sequence number to each
